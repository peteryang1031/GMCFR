...
from ..optim import cg, cg_our, line_search_armijo, solve_1d_linesearch_quad
...
def fused_gromov_wasserstein(M, C_1, C_2, C_Phi_1, C_Phi_2, p=None, q=None, loss_fun='square_loss', symmetric=None, alpha=0.5, beta=0.5,
                             armijo=False, G0=None, log=False, max_iter=1e4, tol_rel=1e-9, tol_abs=1e-9, **kwargs):

    arr = [C_1, C_2, C_Phi_1, C_Phi_2, M]
    if p is not None:
        arr.append(list_to_array(p))
    else:
        p = unif(C_1.shape[0], type_as=C_1)
    if q is not None:
        arr.append(list_to_array(q))
    else:
        q = unif(C_2.shape[0], type_as=C_2)
    if G0 is not None:
        G0_ = G0
        arr.append(G0)

    nx = get_backend(*arr)
    p0, q0, C10, C20, CP10, CP20, M0, alpha0 = p, q, C_1, C_2, C_Phi_1, C_Phi_2, M, alpha

    p = nx.to_numpy(p0)
    q = nx.to_numpy(q0)
    C_1 = nx.to_numpy(C10)
    C_2 = nx.to_numpy(C20)
    C_Phi_1 = nx.to_numpy(CP10)
    C_Phi_2 = nx.to_numpy(CP20)
    M = nx.to_numpy(M0)
    alpha = nx.to_numpy(alpha0)

    if symmetric is None:
        symmetric = np.allclose(C_1, C_1.T, atol=1e-10) and np.allclose(C_2, C_2.T, atol=1e-10) \
        and np.allclose(C_Phi_1, C_Phi_1.T, atol=1e-10) and np.allclose(C_Phi_2, C_Phi_2.T, atol=1e-10)
    ...
    # cg for GW is implemented using numpy on CPU
    np_ = NumpyBackend()
    constC_1, hC1_1, hC2_1 = init_matrix(C_Phi_1, C_Phi_2, p, q, loss_fun, np_)
    constC_2, hC1_2, hC2_2 = init_matrix(C_1, C_Phi_2, p, q, loss_fun, np_)
    constC_3, hC1_3, hC2_3 = init_matrix(C_Phi_1, C_2, p, q, loss_fun, np_)

    def f(G):
        return beta * gwloss(constC_1, hC1_1, hC2_1, G, np_) + - alpha * gwloss(constC_2, hC1_2, hC2_2, G, np_) \
            + - alpha * gwloss(constC_3, hC1_3, hC2_3, G, np_)

    # if symmetric:
    def df(G):
        return beta * gwggrad(constC_1, hC1_1, hC2_1, G, np_) + - alpha * gwggrad(constC_2, hC1_2, hC2_2, G, np_) \
                + - alpha * gwggrad(constC_3, hC1_3, hC2_3, G, np_)
    ...
    if armijo:
        ...
        def line_search(cost, G, deltaG, Mi, cost_G, **kwargs):
            return solve_gromov_linesearch(G, deltaG, cost_G, C_1, C_2, C_Phi_1, C_Phi_2, M= alpha * M, reg1=alpha, reg2=beta, nx=np_, **kwargs)
    if log:
        res, log = cg_our(p, q, (1 - beta) * M, alpha, f, df, G0, line_search, log=True, numItermax=max_iter, stopThr=tol_rel, stopThr2=tol_abs, **kwargs)
        log['fgw_dist'] = nx.from_numpy(log['loss'][-1], type_as=C10)
        log['u'] = nx.from_numpy(log['u'], type_as=C10)
        log['v'] = nx.from_numpy(log['v'], type_as=C10)
        return nx.from_numpy(res, type_as=C10), log
    else:
        return nx.from_numpy(cg_our(p, q, (1 - beta) * M, alpha, f, df, G0, line_search, log=False, numItermax=max_iter, stopThr=tol_rel, stopThr2=tol_abs, **kwargs), type_as=C10)


def fused_gromov_wasserstein2(M, C_1, C_2, C_Phi_1, C_Phi_2, p=None, q=None, loss_fun='square_loss', symmetric=None, alpha=0.5, beta=0.5,
                              armijo=False, G0=None, log=False, max_iter=1e4, tol_rel=1e-9, tol_abs=1e-9, **kwargs):
    nx = get_backend(C_1, C_2, C_Phi_1, C_Phi_2, M)
    ...
    T, log_fgw = fused_gromov_wasserstein(
        M, C_1, C_2, C_Phi_1, C_Phi_2, p, q, loss_fun, symmetric, alpha, beta, armijo, G0, log=True,
        max_iter=max_iter, tol_rel=tol_rel, tol_abs=tol_abs, **kwargs)
    ...
    if loss_fun == 'square_loss':
        ...
        gC_1, gC_2, gC_Phi_1, gC_Phi_2 = 0, 0, 0, 0
        gC_Phi_1 += beta * (2 * C_Phi_1 * nx.outer(p, p) - 2 * nx.dot(T, nx.dot(C_Phi_2, T.T)))
        gC_Phi_2 += beta * (2 * C_Phi_2 * nx.outer(q, q) - 2 * nx.dot(T.T, nx.dot(C_Phi_1, T)))
        
        gC_1 += - alpha * (2 * C_1 * nx.outer(p, p) - 2 * nx.dot(T, nx.dot(C_Phi_2, T.T)))
        gC_Phi_2 += - alpha * (2 * C_Phi_2 * nx.outer(q, q) - 2 * nx.dot(T.T, nx.dot(C_1, T)))
        
        gC_Phi_1 += - alpha * (2 * C_Phi_1 * nx.outer(p, p) - 2 * nx.dot(T, nx.dot(C_2, T.T)))
        gC_2 += - alpha * (2 * C_2 * nx.outer(q, q) - 2 * nx.dot(T.T, nx.dot(C_Phi_1, T)))
        
        fgw_dist = nx.set_gradients(fgw_dist, (p, q, C_1, C_2, C_Phi_1, C_Phi_2, M),
...
def solve_gromov_linesearch(G, deltaG, cost_G, C_1, C_2, C_Phi_1, C_Phi_2, M, reg1, reg2,
    ...
    """
    if nx is None:
        G, deltaG, C_1, C_2, C_Phi_1, C_Phi_2, M = list_to_array(G, deltaG, C_1, C_2, C_Phi_1, C_Phi_2, M)

        if isinstance(M, int) or isinstance(M, float):
            nx = get_backend(G, deltaG, C_1, C_2, C_Phi_1, C_Phi_2)
        else:
            nx = get_backend(G, deltaG, C_1, C_2, C_Phi_1, C_Phi_2, M)
            
    dot_1 = nx.dot(nx.dot(C_Phi_1, deltaG), C_Phi_2.T)
    a_1 = reg2 * -2 * nx.sum(dot_1 * deltaG)
    b_1 = reg2 * -2 * (nx.sum(dot_1 * G) + nx.sum(nx.dot(nx.dot(C_Phi_1, G), C_Phi_2.T) * deltaG))
    
    dot_2 = nx.dot(nx.dot(C_1, deltaG), C_Phi_2.T)
    a_2 = - reg1 * -2 * nx.sum(dot_2 * deltaG)
    b_2 = - reg1 * -2 * (nx.sum(dot_2 * G) + nx.sum(nx.dot(nx.dot(C_1, G), C_Phi_2.T) * deltaG))
    
    dot_3 = nx.dot(nx.dot(C_Phi_1, deltaG), C_2.T)
    a_3 = - reg1 * -2 * nx.sum(dot_3 * deltaG)
    b_3 = - reg1 * -2 * (nx.sum(dot_3 * G) + nx.sum(nx.dot(nx.dot(C_Phi_1, G), C_2.T) * deltaG))

    a = a_1 + a_2 + a_3
    b = nx.sum(M * deltaG) + b_1 + b_2 + b_3
