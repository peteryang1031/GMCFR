...
def entropic_fused_gromov_wasserstein(
        M, C_1, C_2, C_Phi_1, C_Phi_2, p=None, q=None, loss_fun='square_loss', epsilon=0.1,
        symmetric=None, alpha=0.5, beta=0.5, G0=None, max_iter=1000, tol=1e-9,
    ...
    **kwargs: dict
    ...
    M, C_1, C_2, C_Phi_1, C_Phi_2 = list_to_array(M, C_1, C_2, C_Phi_1, C_Phi_2)
    arr = [M, C_1, C_2, C_Phi_1, C_Phi_2]
    if p is not None:
        arr.append(list_to_array(p))
    else:
    ...
    constC_1, hC1_1, hC2_1 = init_matrix(C_Phi_1, C_Phi_2, p, q, loss_fun, nx)
    constC_2, hC1_2, hC2_2 = init_matrix(C_1, C_Phi_2, p, q, loss_fun, nx)
    constC_3, hC1_3, hC2_3 = init_matrix(C_Phi_1, C_2, p, q, loss_fun, nx)
    
    if symmetric is None:
        symmetric = nx.allclose(C_1, C_1.T, atol=1e-10) and nx.allclose(C_2, C_2.T, atol=1e-10) \
            and nx.allclose(C_Phi_1, C_1.T, atol=1e-10) and nx.allclose(C_Phi_2, C_Phi_2.T, atol=1e-10)
    ...
    # if warmstart:
    #     # initialize potentials to cope with ot.sinkhorn initialization
    #     N1, N2 = C_1.shape[0], C_2.shape[0]
    #     mu = nx.zeros(N1, type_as=C_1) - np.log(N1)
    #     nu = nx.zeros(N2, type_as=C_2) - np.log(N2)
    ...
    while (err > tol and cpt < max_iter):
        ...
        # compute the gradient
        tens = (1 - alpha + 2 * beta) * gwggrad(constC_1, hC1_1, hC2_1, T, nx) + -1 * beta * gwggrad(constC_2, hC1_2, hC2_2, T, nx) + \
            -1 * beta * gwggrad(constC_3, hC1_3, hC2_3, T, nx) + alpha * M
        # if symmetric:
    ...
    if abs(nx.sum(T) - 1) > 1e-5:
        warnings.warn("Solver failed to produce a transport plan. You might "
                      "want to increase the regularization parameter `epsilon`.")
    if log:
        log['fgw_dist'] = alpha * nx.sum(M * T) + (1 - alpha + 2 * beta) * gwloss(constC_1, hC1_1, hC2_1, T, nx) \
            + -1 * beta * gwloss(constC_2, hC1_2, hC2_2, T, nx) + -1 * beta * gwloss(constC_3, hC1_3, hC2_3, T, nx)
        return T, log
    else:
        return T
...
def entropic_fused_gromov_wasserstein2(
    ...
    T, logv = entropic_fused_gromov_wasserstein(
        M, C_1, C_2, C_Phi_1, C_Phi_2, p, q, loss_fun, epsilon, symmetric, alpha, beta, G0, max_iter,
        tol, solver, warmstart, verbose, log=True, **kwargs)
