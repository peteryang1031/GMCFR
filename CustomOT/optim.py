...
def generic_conditional_gradient_our(a, b, M, f, df, reg1, reg2, lp_solver, line_search, G0=None,
    ...
    **kwargs : dict
    ...
    if reg2 is None:
        def cost(G):
            return nx.sum(M * G) + f(G)
    else:
        def cost(G):
            return nx.sum(M * G) + f(G) + reg2 * nx.sum(G * nx.log(G))
    cost_G = cost(G)
    if log:
...
def cg_our(a, b, M, reg, f, df, G0=None, line_search=line_search_armijo,
       numItermax=200, numItermaxEmd=100000, stopThr=1e-9, stopThr2=1e-9,
       verbose=False, log=False, **kwargs):
    r"""
    Solve the general regularized OT problem with conditional gradient
    ...
    **kwargs : dict
    ...
    return generic_conditional_gradient_our(a, b, M, f, df, reg, None, lp_solver, line_search, G0=G0,
                                        numItermax=numItermax, stopThr=stopThr,
                                        stopThr2=stopThr2, verbose=verbose, log=log, **kwargs)
