...
def cal_wass(x_0, x_1, rep_0, rep_1, hparams):
    ...
    loss = 0    
    delta_0 = (c_x_0 - c_rep_0)
    delta_1 = (c_x_1 - c_rep_1)
    
    loss += hparams.get('alpha') * torch.sqrt(torch.mean(torch.square(delta_0))) + \
        hparams.get('alpha') * torch.sqrt(torch.mean(torch.square(delta_1)))
...
class BaseEstimator:
    ...
    def fit(self):
        ...
        for epoch in tqdm(range(1, self.epochs)):
            ...
            for batch_idx, data in enumerate(self.train_loader):
                ...
                _x, _xt, _t, _yf, _ = data[:, 5:], data[:, new_idx], data[:, 0], data[:, 1], data[:, 2]  #
    ...
    def predict(self, dataloader):
        ...
        for data in dataloader:
            ...
            idx = np.arange(5, 30)
            new_idx = np.concatenate((idx, [0]))
            _x, _xt, _t, _mu_0, _mu_1, _yf, _ycf = data[:, 5:], data[:, new_idx], data[:, [0]], data[:, 3], data[:, 4], data[:, 1], data[:, 2]  #
...
if __name__ == "__main__":
    ...
    hparams.add_argument('--alpha', type=float, default=0.5, help='trade-off inside Gromov Monge Gap')
